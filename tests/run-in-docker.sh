#!/usr/bin/env bash
#
# Build and run mod-stat-booster's unit tests inside the AzerothCore `ac-dev-server`
# container, isolated from the live stack. The build tree and ccache live in
# Docker volumes (ac-build-dev / ac-ccache-dev), so nothing lands in the project
# folder and the running server is untouched.
#
# Run from the repo root:
#   docker compose --profile dev run --rm ac-dev-server \
#       bash modules/mod-stat-booster/tests/run-in-docker.sh [JOBS]
#
...
./var/build/src/test/unit_tests --gtest_filter="StatBooster*" \
  2>&1 | tee modules/mod-stat-booster/tests/_lastrun.log
